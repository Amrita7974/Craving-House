...
const ResturantCoreDetails = () => {
  ...
  const handleSaveRestaurant = async () => {
    try {
      ...
      const payload = {
        restaurantName: restaurantFormData.restaurantName || restaurantData?.restaurantName,
        ...
        description: restaurantFormData.description || restaurantData?.description,
        restaurantType: restaurantFormData.restaurantType || restaurantData?.restaurantType,
        cuisineTypes: restaurantFormData.cuisineTypes
          ? restaurantFormData.cuisineTypes.split(",").map((c) => c.trim())
          : (restaurantData?.cuisineTypes || []),
        isOpen: restaurantFormData.isOpen,
        contactDetails: {
          email: restaurantFormData.contactEmail || restaurantData?.contactDetails?.email,
          phone: restaurantFormData.contactPhone || restaurantData?.contactDetails?.phone,
        },
        servingHours: {
          openingTime: restaurantFormData.openingTime || restaurantData?.servingHours?.openingTime,
          closingTime: restaurantFormData.closingTime || restaurantData?.servingHours?.closingTime,
        },
        geoLocation: {
        ...
        documents: {
          legalName: restaurantData?.documents?.legalName || "N/A",
          companyType: restaurantData?.documents?.companyType || "N/A",
          gstCertificate: restaurantFormData.gst,
          fssaiCertificate: restaurantFormData.fssai,
          panCard: restaurantFormData.panCard,
        },
        socialMediaLinks: restaurantFormData.socialMediaLinks,
        restaurantImage: restaurantData?.restaurantImage || [{ url: "https://placehold.co/600x400?text=R", publicId: "dummy" }],
        coverImage: restaurantData?.coverImage || { url: "https://placehold.co/600x400?text=R", publicId: "dummy" },
      };

      const res = await api.post("/restaurant/update-profile", payload);
      toast.success(res.data.message || "Restaurant details updated successfully");
  ...
  useEffect(() => {
    if (!user?._id) return;
    const load = async () => {
      // Defer state update to bypass react-hooks/set-state-in-effect
  ...
  useEffect(() => {
    if (restaurantData) {
      // Defer state updates to next microtask queue to avoid cascading render lint rule trigger
